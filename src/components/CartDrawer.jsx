      ...
      {isCartOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white border-l border-[#0e1a30]/5 shadow-2xl z-50 flex flex-col"
          >
            {/* Header */}
            <div className="p-6 border-b border-[#0e1a30]/5 flex justify-between items-center bg-[#fbf9f4]">
              <div className="flex items-center gap-2">
                <FiShoppingBag className="text-[#c5a059] text-xl" />
                <h3 className="text-xl font-bold font-serif text-[#0e1a30]" style={{ fontFamily: "'Cinzel', serif" }}>
              ...
              <button
                onClick={handleClose}
                className="p-2 rounded-full hover:bg-black/5 text-gray-500 hover:text-[#0e1a30] transition"
              >
                <FiX size={20} />
              </button>
            </div>
            ...
            <div className="flex-grow overflow-y-auto p-6">
                ...
                <div className="space-y-4">
                  {cartItems.length === 0 ? (
                    <div className="h-full py-20 flex flex-col items-center justify-center text-center space-y-4">
                      ...
                      <h4 className="text-lg font-bold text-[#0e1a30]">Your cart is empty</h4>
                      <p className="text-sm text-gray-600 max-w-[250px]">
                        Looks like you haven't added any Ayurvedic formulations yet.
                      </p>
                      <button
                        onClick={handleClose}
                        className="px-6 py-2 border border-[#0e1a30] text-[#0e1a30] font-bold rounded-full hover:bg-[#0e1a30] hover:text-white transition duration-300"
                      >
                        Start Shopping
                      </button>
                    </div>
                    ...
                    cartItems.map((item) => (
                      <div
                        key={item.id}
                        className="flex gap-4 p-4 rounded-2xl bg-[#fbf9f4] border border-[#0e1a30]/5 items-center justify-between shadow-sm"
                      >
                        <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#c5a059] to-[#9c772c] shrink-0" />
                        <div className="flex-grow min-w-0">
                          <h4 className="text-sm font-bold text-[#0e1a30] truncate">{item.name}</h4>
                          <p className="text-xs text-gray-600 line-clamp-1">{item.description}</p>
                          <p className="text-sm text-[#c5a059] font-semibold mt-1">₹{item.price}</p>
                        </div>
                        <div className="flex flex-col items-end gap-3">
                          ...
                          <div className="flex items-center bg-white border border-[#0e1a30]/10 rounded-lg p-1">
                            <button
                              onClick={() => updateQuantity(item.id, item.quantity - 1)}
                              className="p-1 hover:text-[#c5a059] transition text-gray-500"
                            >
                              <FiMinus size={12} />
                            </button>
                            <span className="px-2 text-xs font-semibold text-[#0e1a30]">{item.quantity}</span>
                            <button
                              onClick={() => updateQuantity(item.id, item.quantity + 1)}
                              className="p-1 hover:text-[#c5a059] transition text-gray-500"
                            >
              ...
              {checkoutStep === "shipping" && (
                <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                  <h4 className="text-sm font-bold text-[#c5a059] uppercase tracking-wider flex items-center gap-1.5">
                    <FiTruck />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-[#0e1a30]/10 rounded-xl px-4 py-2.5 text-sm text-[#0e1a30] focus:outline-none focus:border-[#c5a059] transition"
                    />
                  </div>
                  <div>
              ...
              {checkoutStep === "success" && (
                <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <h4 className="text-xl font-bold text-[#0e1a30] font-serif" style={{ fontFamily: "'Cinzel', serif" }}>
                    Order Confirmed!
                  </h4>
                  ...
                  <button
                    onClick={handleClose}
                    className="px-6 py-2.5 bg-[#0e1a30] text-white font-bold rounded-xl hover:bg-[#c5a059] transition duration-200 shadow-md"
                  >
                    Continue Shopping
                  </button>
                </div>
            ...
            {checkoutStep !== "success" && cartItems.length > 0 && (
              <div className="p-6 border-t border-[#0e1a30]/10 bg-[#fbf9f4] space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Total Amount</span>
                  <span className="text-2xl font-bold text-[#c5a059]">₹{cartTotal}</span>
                </div>

                {checkoutStep === "cart" ? (
                  <button
                    onClick={handleProceedToCheckout}
                    className="w-full py-3.5 bg-[#0e1a30] text-white font-black rounded-xl hover:bg-[#c5a059] active:scale-[0.98] transition duration-200 shadow-md"
                  >
                    Proceed to Checkout
                  </button>
                ) : (
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full py-3.5 bg-[#0e1a30] text-white font-black rounded-xl hover:bg-[#c5a059] active:scale-[0.98] transition duration-200 disabled:opacity-50 flex items-center justify-center gap-2 shadow-md"
                    >
                    ...
                    <button
                      ...
                      disabled={loading || !shippingDetails.fullName || !shippingDetails.address || !shippingDetails.phone}
                      className="w-full py-2 bg-transparent text-gray-600 border border-[#0e1a30]/10 hover:text-[#0e1a30] rounded-xl text-xs font-semibold hover:bg-[#0e1a30]/5 transition duration-200 disabled:opacity-50"
                    >
