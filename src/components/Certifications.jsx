...
export default function Certifications() {
  const certs = [
    {
      ...
      icon: (
        <svg className="w-10 h-10 text-[#c5a059]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
  ...
  return (
    <section className="bg-[#fbf9f4] py-12 border-y border-[#c5a059]/10 relative overflow-hidden">
      {/* Dynamic line glow */}
      ...
      <div className="max-w-7xl mx-auto px-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 items-center text-center">
          {certs.map((cert, index) => (
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center border border-[#0e1a30]/5 transition-all duration-300 group-hover:border-[#c5a059]/30 group-hover:bg-white group-hover:shadow-[0_8px_30px_rgba(14,26,48,0.06)] group-hover:scale-105 shadow-sm">
                {cert.icon}
              </div>
              <div>
                <h4 className="text-sm font-bold text-[#0e1a30] tracking-wide group-hover:text-[#c5a059] transition-colors duration-200">
                  {cert.title}
                </h4>
